  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (isValid) {
        ...
        // Form can be submitted here or handled via AJAX
        // contactForm.submit();
      }
    });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "שירה מזרחי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: 'df56048e-6dfc-4ba8-9edd-0cde6d59bdec',
                    name: data.name || '',
                    email: data.email || '',
